...
export default function App() {
  ...
  function handleAddWatched(movie) {
    //alternative for not adding the same movie twice
    // if (watched.some((watchedMovie) => watchedMovie.imdbID === movie.imdbID)) {
    //   console.log("Movie already in list");
    //   return;
    // }
    setWatched((watched) => [...watched, movie]);
    //not great way to add movie to watched list with local storage
...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  useEffect(
    function () {
      if (!title) return; //if title is not set yet, do nothing
      document.title = `Movie | ${title}`;
      //cleanup function
      return function () {
        document.title = "usePopcorn"; //reset title
      };
    },
    [title]
  );
